/**
 * Métadonnées du module Python.
 * Contenu extrait et traduit depuis "Python Notes for Professionals"
 * (GoalKicker.com) — voir python/pdf.html pour consulter la source.
 */
var CI = (path) => `<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">${path}</svg>`;

var CHAPTERS_META = [
  {
    id: "01",
    slug: "prise-en-main",
    title: "Prise en main de Python",
    short: "Installer Python, exécuter son premier script, comprendre l'interpréteur.",
    icon: CI('<path d="M5 3v18l14-9L5 3Z"/>'),
    minutes: 10,
    quizCount: 6,
  },
  {
    id: "02",
    slug: "types-de-donnees",
    title: "Les types de données",
    short: "Entiers, décimaux, texte, booléens : les briques de base d'une valeur.",
    icon: CI('<path d="M3 7a2 2 0 0 1 2-2h7l7 7-7 7H5a2 2 0 0 1-2-2V7Z"/><circle cx="15.5" cy="12" r="1.3" fill="currentColor" stroke="none"/>'),
    minutes: 10,
    quizCount: 6,
  },
  {
    id: "03",
    slug: "indentation",
    title: "L'indentation",
    short: "La particularité de Python : l'indentation remplace les accolades.",
    icon: CI('<path d="M4 6h16M8 12h12M4 18h16"/>'),
    minutes: 8,
    quizCount: 5,
  },
  {
    id: "04",
    slug: "commentaires",
    title: "Commentaires et documentation",
    short: "Documenter son code pour soi-même et pour les autres.",
    icon: CI('<path d="M4 4h16v12H8l-4 4V4Z"/>'),
    minutes: 6,
    quizCount: 4,
  },
  {
    id: "05",
    slug: "operateurs",
    title: "Opérateurs mathématiques et booléens",
    short: "Calculer, comparer et combiner des conditions.",
    icon: CI('<path d="M12 5v6M9 8h6M6 17h5M15 17h3"/>'),
    minutes: 12,
    quizCount: 7,
  },
  {
    id: "06",
    slug: "comparaisons-conditions",
    title: "Comparaisons et conditions",
    short: "Prendre des décisions avec if, elif et else.",
    icon: CI('<circle cx="6" cy="6" r="2.4"/><circle cx="6" cy="18" r="2.4"/><circle cx="17" cy="12" r="2.4"/><path d="M6 8.4V15.6M8.2 6.9 14.8 11M8.2 17.1 14.8 13"/>'),
    minutes: 12,
    quizCount: 7,
  },
  {
    id: "07",
    slug: "boucles",
    title: "Les boucles",
    short: "Répéter des instructions avec while et for.",
    icon: CI('<path d="M3 11a9 9 0 0 1 15-6.7L21 7M21 3v5h-5M21 13a9 9 0 0 1-15 6.7L3 17m0 4v-5h5"/>'),
    minutes: 14,
    quizCount: 8,
  },
  {
    id: "08",
    slug: "listes",
    title: "Les listes",
    short: "Stocker, parcourir et modifier une collection ordonnée de valeurs.",
    icon: CI('<rect x="2.5" y="8" width="19" height="8" rx="1.5"/><path d="M9 8v8M14.5 8v8"/>'),
    minutes: 16,
    quizCount: 8,
  },
  {
    id: "09",
    slug: "dictionnaires",
    title: "Les dictionnaires",
    short: "Associer une clé à une valeur pour retrouver l'information instantanément.",
    icon: CI('<path d="M8 3.5c-2 0-3 1-3 3v2.7c0 1.2-.6 2.3-2 2.8 1.4.5 2 1.6 2 2.8v2.7c0 2 1 3 3 3M16 3.5c2 0 3 1 3 3v2.7c0 1.2.6 2.3 2 2.8-1.4.5-2 1.6-2 2.8v2.7c0 2-1 3-3 3"/>'),
    minutes: 14,
    quizCount: 7,
  },
  {
    id: "10",
    slug: "entrees-sorties",
    title: "Entrées-sorties",
    short: "Afficher un résultat et récupérer une saisie utilisateur.",
    icon: CI('<path d="M21 11.5a8.5 8.5 0 0 1-8.5 8.5c-1.35 0-2.6-.32-3.7-.9L3 21l1.9-5.7A8.44 8.44 0 0 1 3.5 11.5 8.5 8.5 0 0 1 12 3a8.5 8.5 0 0 1 9 8.5Z"/>'),
    minutes: 8,
    quizCount: 5,
  },
  {
    id: "11",
    slug: "fichiers",
    title: "Manipuler des fichiers",
    short: "Ouvrir, lire et écrire dans un fichier avec open() et with.",
    icon: CI('<path d="M3 7.2A2.2 2.2 0 0 1 5.2 5h3.6l1.8 2H19a2 2 0 0 1 2 2v8a2.2 2.2 0 0 1-2.2 2.2H5.2A2.2 2.2 0 0 1 3 17V7.2Z"/>'),
    minutes: 12,
    quizCount: 6,
  },
  {
    id: "12",
    slug: "fonctions",
    title: "Les fonctions",
    short: "Découper son code en blocs réutilisables avec def et return.",
    icon: CI('<path d="M10 3.5a1.8 1.8 0 0 1 3.6 0V5H16a1 1 0 0 1 1 1v2.4h1.5a1.8 1.8 0 0 1 0 3.6H17V14a1 1 0 0 1-1 1h-2.4v1.5a1.8 1.8 0 0 1-3.6 0V15H7a1 1 0 0 1-1-1v-2.4H4.5a1.8 1.8 0 0 1 0-3.6H6V6a1 1 0 0 1 1-1h3V3.5Z"/>'),
    minutes: 16,
    quizCount: 8,
  },
  {
    id: "13",
    slug: "exceptions",
    title: "Gérer les erreurs (exceptions)",
    short: "Anticiper et traiter proprement les erreurs d'exécution avec try/except.",
    icon: CI('<path d="M10.29 3.86 1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0Z"/><path d="M12 9v4M12 17h.01"/>'),
    minutes: 12,
    quizCount: 6,
  },
  {
    id: "14",
    slug: "classes",
    title: "Introduction aux classes",
    short: "Premiers pas avec la programmation orientée objet en Python.",
    icon: CI('<path d="M12 3 3 8v8l9 5 9-5V8l-9-5Z"/><path d="M3 8l9 5 9-5M12 13v8"/>'),
    minutes: 16,
    quizCount: 7,
  },
];

var TOTAL_QUIZ_QUESTIONS = CHAPTERS_META.reduce((s, c) => s + c.quizCount, 0);

function chapterById(id) {
  return CHAPTERS_META.find((c) => c.id === id);
}
function chapterIndex(id) {
  return CHAPTERS_META.findIndex((c) => c.id === id);
}

window.MODULE_CHAPTERS = window.MODULE_CHAPTERS || {};
window.MODULE_CHAPTERS["python"] = CHAPTERS_META;
